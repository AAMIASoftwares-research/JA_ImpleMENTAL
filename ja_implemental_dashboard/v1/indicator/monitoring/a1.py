    ...
    def get_plot(self, **kwargs):
        ...
        plot_all = (
            holoviews.Curve(
                ...
                label=DISEASES_LANGDICT[language_code]["_all_"]
            ).opts(  
                color="#a0a0a0ff" 
            ) * holoviews.Scatter(
                (years_to_evaluate, ma1_all),
                kdims=[("year", _year_langdict[language_code])],
                vdims=[("number_of_patients", _number_of_patients_langdict[language_code])],
                label=DISEASES_LANGDICT[language_code]["_all_"]
            ).opts(
                size=10,
                color="#a0a0a0ff", 
        ...
        plot_disease = (
            holoviews.Curve(
                ...
                label=DISEASES_LANGDICT[language_code][disease_code]
            ).opts(
                color="#FF204Eff" # https://colorhunt.co/palette/ff204ea0153e5d0e4100224d
        ...
        plot = (
            plot_all * plot_disease
        ).opts(
            title=ma1_code + " - " + ma1_name_langdict[language_code] + " - " + DISEASES_LANGDICT[language_code][disease_code],
            xlabel=_year_langdict[language_code],
            xticks=numpy.sort(years_to_evaluate),
            xlim=(years_to_evaluate[0]-0.5, years_to_evaluate[-1]+0.5),
            ylabel=_number_of_patients_langdict[language_code],
            legend_position='bottom_right',
        )
        out = panel.pane.HoloViews(plot)
        return out
